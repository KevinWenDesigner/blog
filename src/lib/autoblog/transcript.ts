...
function normalizeRequestedSubtitles(
  tracks: Record<string, SubtitleTrack | null | undefined> | undefined
): Record<string, SubtitleTrack[]> | undefined {
  const entries = Object.entries(tracks ?? {})
    .filter((entry): entry is [string, SubtitleTrack] => Boolean(entry[1]?.url))
    .map(([language, track]) => [language, [track]] as const);
...
export function selectSubtitleTrack(metadata: VideoMetadata): SubtitleTrackSelection | null {
  ...
  for (const [language, tracks] of sortedLanguageEntries(
    normalizeRequestedSubtitles(metadata.requested_subtitles),
    ORIGINAL_AUTOMATIC_LANGUAGE_PRIORITY
  )) {
    const selected = pickAutomaticTrack(tracks);
    if (selected?.url) {
